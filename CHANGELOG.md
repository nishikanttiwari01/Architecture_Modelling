...
- Added, rendered and visually inspected corrected `FIG-35-01` at native 690 by 814 pixels after its separate specification checkpoint; distinct row-aligned bands separate stages, capabilities and candidate BIAN analysis, and the figure remains at `Review`.
...
- Regenerated `FIG-27-01` from the final landscape specification and recorded separate SVG and PNG dimensions, native-size inspection, and fresh results for all six required repository checks.
...
- Added, specified, rendered, visually reviewed and integrated `FIG-20-01`; its register
  status is `Review`, not `Approved`.
