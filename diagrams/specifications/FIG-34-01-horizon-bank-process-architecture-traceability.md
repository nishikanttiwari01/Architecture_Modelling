...
PlantUML produced native 711 by 861 pixel SVG and PNG exports without post-render scaling. SVG inspection confirms 12-pixel node text, 11-pixel relationship text and a 14-pixel title. Original-resolution and intended-page-width review found no clipped node text, overlapping ownership label or ambiguous arrow direction. Hidden vertical layout links carry no business meaning. The process is the source of `produces`, `is constrained by`, `is measured by`, `exercises`, `is informed by` and `details selected behaviour`; the owner sits above and points vertically to the process with `is accountable for`. The selected scope, BIAN caution and chapter caption agree. Status remains `Review`.
