...
| 14 | Modelling Business Strategy and Capabilities | Drafting | 100 | 100 | 20 | 0 | 100 | Author approval of `FIG-14-01` and `FIG-14-02` specifications, then produce figures |
...
Chapter 14, **Modelling Business Strategy and Capabilities**, has been drafted and is now `Drafting`. The full prose is complete for every planned section: goals, drivers and outcomes; stakeholder concerns; capability maps for planning; value streams and the value-stream-to-capability cross-map; investment heat maps; traceability to initiatives; recommended notation combinations; a selection table; a Horizon Bank worked example; common mistakes; and the chapter endings. As the first Part III chapter, it selects and combines strategy and capability models rather than re-teaching the notations, and it references the Chapter 7 and Chapter 13 figures. It reuses only registered sources (`[OPEN-GROUP-ARCHIMATE-4]`, `[OPEN-GROUP-BIZARCH-GUIDES-2022]`, `[ISO-42010]`) and the controlled example names in `examples/horizon-bank/capabilities.md` and `examples/simple-online-store/README.md`, so no new source note was required. Two figures are registered and specified: `FIG-14-01` Strategy-to-Capability Traceability View and `FIG-14-02` Onboarding Value Stream to Capability Cross-Map. Their editable sources and SVG exports are deliberately deferred (status `Planned`, source `Pending`) until the author approves the specifications, per the diagram workflow; each figure's content is presented as an inline table so the chapter reads completely. The next action is author approval of the two figure specifications, after which the figures can be produced and moved to `Review`, followed by the formal review passes. No chapter or figure is marked `Approved`.
...
Last updated: 2026-07-03 (Chapter 14 drafted)
