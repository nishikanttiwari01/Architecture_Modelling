...
| 13 | Other Useful Modelling Approaches | Diagramming | 100 | 100 | 90 | 60 | 100 | Author review of rendered FIG-13-01 to FIG-13-06 (all Review); Draw.io graphical review and final page layout pending |
...
Chapter 12, **Security Modelling**, is approved by the author with final quality score 9.1. PlantUML source, SVG exports and PNG previews exist for `FIG-12-01`, `FIG-12-02`, `FIG-12-04` and `FIG-12-05`; all four figures remain at `Review` pending final page-layout review, particularly wider figures `FIG-12-01` and `FIG-12-04`. The former `FIG-12-03` access matrix is retired and replaced by manuscript table `TABLE-12-01`. Chapter 13, **Other Useful Modelling Approaches**, is now in `Diagramming`. The author approved the revised `FIG-13-01` through `FIG-13-06` specifications for production, and all six figures now have an editable source, an SVG export and a PNG preview and are recorded as `Review`. `FIG-13-01` uses PlantUML; `FIG-13-02` through `FIG-13-06` use Draw.io exported by `scripts/render-drawio-diagrams.py` (see `DEC-021`). No Chapter 13 figure is `Approved`. A focused correction pass then fixed the `FIG-13-02` trigger-label placement, integrated the Draw.io renderer into the reproducible workflow and CI (syntax check, pinned Pillow dependency in `requirements-diagrams.txt`, full render script and a stale-export check), and added deterministic cross-platform font selection. `DEC-021` is now `Proposed`, not `Approved`: native Draw.io graphical-open and export-fidelity review remain pending, and the previews are not verified native Draw.io exports. A Draw.io graphical review and final book-page layout review remain pending, particularly for the wider figures `FIG-13-01` and `FIG-13-03`. Chapter 14 has not started.
