...
| Status | Meaning |
|---|---|
| Planned | Chapter file and scope exist. |
| Researching | Sources are being collected and verified. |
| Outlined | Detailed section structure is approved. |
| Drafting | Prose is being written. |
| Diagramming | Required figures are being produced. |
| Under Review | One or more formal review passes are active. |
| Revision Required | Review findings require changes. |
| Ready for Author Approval | All quality gates passed; author decision pending. |
| Approved | Explicitly approved by the author. |
| Published | Included in a released edition. |
...
| 60 | Common Modelling Mistakes | Planned | 0 | 0 | 0 | 0 | 0 | — |
| 61 | Modelling Tools and Diagrams as Code | Planned | 0 | 0 | 0 | 0 | 0 | — |
| 62 | Architecture Review Checklists | Planned | 0 | 0 | 0 | 0 | 0 | — |
| 63 | Building and Maintaining an Architecture Repository | Planned | 0 | 0 | 0 | 0 | 0 | — |

## Front matter

| Item | Status | Next action |
|---|---|---|
| Title page | Draft placeholder | Confirm subtitle and author credit |
| Preface | Planned | Draft after prototype chapter |
| How to use this book | Outlined | Refine after manuscript structure stabilises |
| Table of contents | Generated outline | Keep synchronised with `BOOK_PLAN.md` |

## Appendices

| Appendix | Status | Next action |
|---|---|---|
| A: Diagram Selector | Planned | Build after Part III |
| B: UML Quick Reference | Planned | Build after Chapter 4 |
| C: BPMN Quick Reference | Planned | Build after Chapter 6 |
...
Chapter 14, **Modelling Business Strategy and Capabilities**, has completed a full technical, beginner, consistency and final review pass and is now `Revision Required` with an interim quality-gate score of `8.2/10`. The revision made the chapter technically precise and internally consistent before diagram production. Key corrections: FIG-14-01 is reclassified as an ArchiMate-informed strategy traceability teaching view with simplified teaching links, because the licensed ArchiMate 4 specification text is out of scope for AI-assisted relationship verification (recorded in `research/archimate/open-group-archimate-4.md`); outcomes now carry full measurement definitions (`OUT-14-01` to `OUT-14-04`) with clearly illustrative placeholders; the heat map separates strategic importance, current condition, current investment status, proposed priority, delivery constraint and final funding, with no composite score and no implication that priority proves funding; the categorical goal-to-initiative rule is replaced by nuanced guidance; the `Executive Sponsor` controlled role and controlled capability levelling (including the Financial Crime Screening versus Payment Screening peer scoping) are recorded in the example files; stable IDs (`DRV`, `GOAL`, `OUT`, `COA`, `WP`) and one master traceability table are shared by the manuscript and `FIG-14-01`; and `FIG-14-02` is redesigned as a genuine capability-by-stage matrix with an exact dataset. Both figures remain `Planned` with `Pending` source and export; no diagram source or export exists. `DEC-022` is now `Proposed`. Review records exist under `reviews/technical-review/`, `reviews/beginner-review/`, `reviews/consistency-review/` and `reviews/final-review/` (all `chapter-14-review-2026-07-04.md`), with the interim gate at `reviews/chapter-gates/CH-14-quality-gate.md`. The next action is author review of the revision: approve the two figure specifications for production and accept `DEC-022`, after which the figures can be produced and moved to `Review`. Two items are open by design: author approval, and formal ArchiMate relationship verification by a licensed human reviewer. Nothing is marked `Approved`; Chapter 15 was not started.

Chapter 12, **Security Modelling**, is approved by the author with final quality score 9.1. PlantUML source, SVG exports and PNG previews exist for `FIG-12-01`, `FIG-12-02`, `FIG-12-04` and `FIG-12-05`; all four figures remain at `Review` pending final page-layout review, particularly wider figures `FIG-12-01` and `FIG-12-04`. The former `FIG-12-03` access matrix is retired and replaced by manuscript table `TABLE-12-01`. Chapter 13, **Other Useful Modelling Approaches**, has completed its final technical, content, diagram and quality-gate review and is now `Ready for Author Approval` with a final quality score of 9.0. All six figures have an editable source, an SVG export and a PNG preview and are recorded as `Review`; `FIG-13-01` uses PlantUML and `FIG-13-02` through `FIG-13-06` use Draw.io exported by `scripts/render-drawio-diagrams.py` (see `DEC-021`, Proposed). The final review confirmed technical accuracy, beginner comprehension, terminology and example consistency, diagram quality, accessibility, source and copyright, copy-editing and page-fit readability, and removed the reader-facing draft scaffolding on moving to a production status (per `DEC-010`). No Chapter 13 figure is `Approved`. Remaining items are explicit author decisions: author approval of the chapter and figures; native Draw.io graphical-open and export-fidelity comparison (`DEC-021`); and final book-page layout placement of the wider landscape figures `FIG-13-01`, `FIG-13-03` and `FIG-13-04` (`DEC-014`). Chapter 14 has not started.

Last updated: 2026-07-04 (Chapter 14 revised to Revision Required)
