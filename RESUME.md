...
Chapter 13, **Other Useful Modelling Approaches**, is in `Revision Required` after review-driven corrections. The revised draft covers SysML-style requirement traceability, capability maps, value streams, application landscapes, integration landscapes, architecture roadmaps, heat maps, Wardley maps and ADRs. The review pass added controlled Horizon Bank capability and system lifecycle examples, official Open Group business architecture guide source notes, a local heat-map scoring convention note, an original Horizon Bank ADR example, and revised specifications for `FIG-13-01` through `FIG-13-06`. The figures remain registered as `Planned`; no Chapter 13 diagram source or exports have been created because the specifications still need author approval.
