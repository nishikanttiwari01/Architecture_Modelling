...
Select an artefact for each need: rule combinations; dependency on screening and account-status decisions; operational repair steps; the choice between two decision platforms; and comparison criteria for those platforms. Then sketch four representative rules and identify an illustrative, bank-defined owner and authority, missing-input behaviour, hit policy, reason codes and two boundary tests.

Suggested answer: use a decision table for routing rules, a DRD for dependencies, BPMN for repair work, an ADR for the platform choice and a trade-off matrix as supporting evidence. Keep screening logic separate if another owner governs it. For this exercise, use the illustrative, bank-defined roles `Payment Routing Rule Owner` as the decision-table owner and `Payments Operations Policy Authority` as the approving authority. These teaching labels do not prescribe organisational or regulatory roles in any jurisdiction; a real bank must name its accountable roles.
