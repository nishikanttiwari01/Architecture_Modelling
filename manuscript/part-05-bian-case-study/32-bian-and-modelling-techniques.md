...
*Figure 32.1 (`FIG-32-01`). Different techniques answer different questions along one Horizon Bank payment thread. The traceability record qualifies the relationships. No row or arrow asserts that a Service Domain equals a capability, process step, application, interface, data entity, event topic or deployment unit.*
