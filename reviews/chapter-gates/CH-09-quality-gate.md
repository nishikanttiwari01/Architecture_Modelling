...
| Technical accuracy | 9.1 | Preserves OMG DMN 1.5 as normative baseline, records Camunda DMN 1.3 tooling caution and avoids claiming DMN 1.5 conformance from Camunda. |
...
| Diagram quality | 9.0 | Four figures render to SVG and PNG, are readable, use restrained colour and keep all Chapter 9 diagrams at `Review`. |
...
`FIG-09-02` is a general decision tree and is explicitly not presented as a formal DMN DRD element.

`FIG-09-03` shows a DMN-style decision requirements teaching view. It uses a connector for information requirements to keep the view readable. It is not semantic `.dmn` XML.

`FIG-09-04` separates BPMN process responsibility from DMN decision responsibility and states that technical binding is vendor-specific.
