# Chapter 21 Quality Gate Review

- **Chapter:** 21, Modelling Security Architecture
- **Review date:** 2026-07-11
- **Reviewer:** Codex
- **Status:** Ready for Author Approval

| Review area | Result | Notes |
|---|---|---|
| Technical accuracy | Pass | Registered official and primary sources support terminology and framing. |
| Beginner comprehension | Pass | Plain questions precede formal detail. |
| Educational flow | Pass | Selection leads to comparison, worked example and practice. |
| Terminology and consistency | Pass | Aligned with Chapter 12, adjacent chapters and examples. |
| Diagram quality | Pass | SVG and PNG rendered; original PNG inspected; register status is Review. |
| Source and copyright | Pass | Existing notes reused; figure and prose are original. |
| Copy-editing | Pass | British English; no em dash. |

## Quality-gate checklist

- [x] Required sections are complete.
- [x] Claims requiring sources have registered notes.
- [x] Required figure is specified, rendered, inspected, registered and integrated.
- [x] Worked example and exercise are materially distinct.
- [x] Selection guidance, comparison and common mistakes are actionable.
- [x] Terminology and examples are consistent.
- [x] Author-only dependencies are documented.

## Open author decisions

- Review `FIG-21-01` and decide whether it may be marked `Approved`.
- Accept, revise or reject `DEC-029`.
- Review Chapter 21 and decide whether to mark it `Approved`.
